...
void vTask_HMI_Display(void* pvParameters) {
    // Initialize the display class and start UART communication
    DisplayHMI HMI_display;
    HMI_display.begin();

    // Temporary test variables (Future: Real CAN data will be mapped here)
    uint8_t HMI_currentSpeed = 0;
    ...
    while (true) {
        ...
        // Update the screen with current speed and battery values
        HMI_display.updateScreen(HMI_currentSpeed, HMI_currentBattery);

        // Check if the driver touched a button on the screen
        if (HMI_display.readTouchCommand(HMI_incomingCommand)) {
            // TODO: Send command to VcuLogic based on the incoming touch command
            // (e.g., VcuLogic::postEvent(VcuLogic::VcuEvent::START_REQUEST);)
        }
